...
from course.forms import CourseForm, ScheduleForm, BatchGradeForm
...
def batch_grade(request, course_id):
    user = get_user(request, "teacher")
    if not user:
        return redirect(reverse("login", kwargs={"kind": "teacher"}))

    course = Course.objects.get(pk=course_id)
    if course.status != 4:  # Only allow grading when course is in grading status
        return redirect(reverse("view_detail", kwargs={"course_id": course_id}))
    ...
    if request.method == 'POST':
        ...
        for cs in student_courses:
            student_id = f"{cs.student.grade}{cs.student.number}"
            ...
            if score:
                try:
                    ...
                    if 0 <= score_float <= 100:
                        cs.scores = score_float
                        cs.comments = comment
                        cs.save()
                except ValueError:
                    pass  # Invalid score format, skip this student
            
        return redirect(reverse("view_detail", kwargs={"course_id": course_id}))

    return render(request, 'course/teacher/batch_grade.html', {
        'info': info,
        'course': course,
        'student_courses': student_courses,
        'return_url': reverse("view_detail", kwargs={"course_id": course_id})
    })
