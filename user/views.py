...
def login(request, *args, **kwargs):  #接收request对象以及可变位置参数args和可变关键字参数kwargs
    if not kwargs or "kind" not in kwargs or kwargs["kind"] not in ["teacher", "student"]:
        return HttpResponse(INVALID_KIND)

    kind = kwargs["kind"]

    if request.method == 'POST':   #get目的是请求服务器返回指定资源的内容，这些资源可以是 HTML 页面、图片、脚本文件等。post主要用于向服务器提交数据，通常用于创建或更新服务器上的资源.
        if kind == "teacher":
